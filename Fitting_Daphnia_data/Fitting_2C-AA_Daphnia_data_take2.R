...
if (!file.exists('results-2C-AA-take2-stage-one.rda')) {
    ...
    save(results, file='~/Dropbox/Cressler/DEB_fitting/Fitting_Daphnia_data/results-2C-AA-take2-stage-one.rda')
} else load('results-2C-AA-take2-stage-one.rda')
...
for (i in 1:length(results)) {
    if (is.data.frame(results[[i]])) {
        ...
        results2[[i]] <- vals
        save(results2, file='~/Dropbox/Cressler/DEB_fitting/Fitting_Daphnia_data/results-2C-AA-take2-stage-two.rda')
    }
}
...
for (i in inds) {
  ...
  colnames(vals) <- c('alpha','kappa','beta','W.scalar','L.scalar','Rm.scalar','time.scalar','f','T_M','W.0','Rm.0','L.0','L.sd','R.sd','loglik','conv','dloglik')
  results3[[i]] <- vals
  save(results3, file='~/Dropbox/Cressler/DEB_fitting/Fitting_Daphnia_data/results-2C-AA-take2-stage-three.rda')
}
