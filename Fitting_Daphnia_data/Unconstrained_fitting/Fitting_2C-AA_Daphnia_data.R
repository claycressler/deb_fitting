...
save(results, file='~/Dropbox/Cressler/DEB_fitting/results-2C-AA-stage-one.rda')
...
for (i in 1:length(results)) {
    if (is.data.frame(results[[i]])) {
        ...
        results2[[i]] <- vals
        save(results2, file='~/Dropbox/Cressler/DEB_fitting/results-2C-AA-stage-two.rda')
    }
}
...
for (i in c(1,3,6)) {
  ...
  for (j in 1:100) {
    ...
    start.loglik <- as.numeric(results2[[i]][j,16])
  ...
  colnames(vals) <- c('alpha','kappa','beta','W.scalar','L.scalar','Rm.scalar','time.scalar','f','T_M','W.0','Lmeas.0','Rm.0','L.0','L.sd','R.sd','loglik','conv','dloglik')
  results3[[i]] <- vals
  save(results3, file='~/Dropbox/Cressler/DEB_fitting/results-2C-AA-stage-three-redo.rda')
}
