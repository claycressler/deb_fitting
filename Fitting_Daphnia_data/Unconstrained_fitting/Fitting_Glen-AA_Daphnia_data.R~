...
save(results, file='~/Dropbox/Cressler/DEB_fitting/results-GLEN-AA-stage-one.rda')
...
for (i in 1:length(results)) {
    if (is.data.frame(results[[i]])) {
        ...
        results2[[i]] <- vals
        save(results2, file='~/Dropbox/Cressler/DEB_fitting/results-GLEN-AA-stage-two.rda')
    }
}
