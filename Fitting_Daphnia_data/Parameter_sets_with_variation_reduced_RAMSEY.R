...
save(fixed.parset, file='Clay_RAMSEY-AA_best_fits_w_variation_reduced_2.rda')
...
for (j in 1:7) {
    ...
    assign(paste0('dCorr.d',par,'.RAMSEY'), delta.pcorr)
}

dCorrdPar <- c(dCorr.dalpha.RAMSEY, dCorr.dkappa.RAMSEY, dCorr.dbeta.RAMSEY, dCorr.dW.scalar.RAMSEY, dCorr.dL.scalar.RAMSEY, dCorr.dRm.scalar.RAMSEY, dCorr.dtime.scalar.RAMSEY)
names(dCorrdPar) <- c('alpha','kappa','beta','W.scalar','L.scalar','Rm.scalar','time.scalar')

save(dCorrdPar, file='Change_in_correlation_with_reduced_variation_RAMSEY_2.rda')
