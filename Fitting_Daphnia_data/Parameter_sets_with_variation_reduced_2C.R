...
for (q in 1:7) {
    ...
    upper <- c(rep(50,7),2,0.2,50,50) ## Watch out for the upper bound on W!
...
save(fixed.parset, file='Clay_2C-AA_best_fits_w_variation_reduced_2.rda')
...
for (j in 1:7) {
    ...
    assign(paste0('dCorr.d',par,'.2C'), delta.pcorr)
}

dCorrdPar <- c(dCorr.dalpha.2C, dCorr.dkappa.2C, dCorr.dbeta.2C, dCorr.dW.scalar.2C, dCorr.dL.scalar.2C, dCorr.dRm.scalar.2C, dCorr.dtime.scalar.2C)
names(dCorrdPar) <- c('alpha','kappa','beta','W.scalar','L.scalar','Rm.scalar','time.scalar')

save(dCorrdPar, file='Change_in_correlation_with_reduced_variation_2C_2.rda')
