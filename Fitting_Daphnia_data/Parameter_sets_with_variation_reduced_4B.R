...
for (q in 1:7) {
    ...
    for (i in 1:nrow(p)) {
        if (!is.na(p[i,1])) {
            ...
            ## log(a) by 1%, change a to a^0.99). For kappa, this is a
...
save(fixed.parset, file='Clay_4B-AA_best_fits_w_variation_reduced_2.rda')
...
save(dCorrdPar, file='Change_in_correlation_with_reduced_variation_4B_2.rda')

load('Change_in_correlation_with_reduced_variation_2C.rda')
dCorrdPar.2C <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_4B.rda')
dCorrdPar.4B <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_GLEN.rda')
dCorrdPar.GLEN <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_RAMSEY.rda')
dCorrdPar.RAMSEY <- dCorrdPar

par(oma=c(4,4,2,2), mar=rep(0,4))
plot.new()
plot.window(xlim=c(0,1), ylim=range(c(dCorrdPar.2C,dCorrdPar.4B,dCorrdPar.GLEN,dCorrdPar.RAMSEY)))
...
dev.copy2pdf(file='Change_in_correlation_from_reducing_variation.pdf')

load('Change_in_correlation_with_reduced_variation_2C_2.rda')
dCorrdPar.2C <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_4B_2.rda')
dCorrdPar.4B <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_GLEN_2.rda')
dCorrdPar.GLEN <- dCorrdPar
load('Change_in_correlation_with_reduced_variation_RAMSEY_2.rda')
dCorrdPar.RAMSEY <- dCorrdPar

par(oma=c(3,5,1,1), mar=rep(0,4))
plot.new()
plot.window(xlim=c(0,1), ylim=range(c(dCorrdPar.2C,dCorrdPar.4B,dCorrdPar.GLEN,dCorrdPar.RAMSEY)))
at.seq <- seq(0.05,0.95,length=7)
axis(1, at=at.seq, labels=c(expression(alpha),expression(kappa),expression(beta),'a', 'b', 'c', 'd'), cex.axis=1.5)
axis(2, cex.axis=1.5)
mtext(side=2, line=3, 'Proportional change in correlation', cex=2)
box('plot')
abline(h=0)
points(at.seq[1], dCorrdPar.4B['alpha'], pch=21, cex=2, bg='green')
points(at.seq[1], dCorrdPar.2C['alpha'], pch=21, cex=2, bg='blue')
points(at.seq[1], dCorrdPar.GLEN['alpha'], pch=21, cex=2, bg='gold')
points(at.seq[1], dCorrdPar.RAMSEY['alpha'], pch=21, cex=2, bg='red')

points(at.seq[2], dCorrdPar.4B['kappa'], pch=21, cex=2, bg='green')
points(at.seq[2], dCorrdPar.2C['kappa'], pch=21, cex=2, bg='blue')
points(at.seq[2], dCorrdPar.GLEN['kappa'], pch=21, cex=2, bg='gold')
points(at.seq[2], dCorrdPar.RAMSEY['kappa'], pch=21, cex=2, bg='red')

points(at.seq[3], dCorrdPar.4B['beta'], pch=21, cex=2, bg='green')
points(at.seq[3], dCorrdPar.2C['beta'], pch=21, cex=2, bg='blue')
points(at.seq[3], dCorrdPar.GLEN['beta'], pch=21, cex=2, bg='gold')
points(at.seq[3], dCorrdPar.RAMSEY['beta'], pch=21, cex=2, bg='red')

points(at.seq[4], dCorrdPar.4B['W.scalar'], pch=21, cex=2, bg='green')
points(at.seq[4], dCorrdPar.2C['W.scalar'], pch=21, cex=2, bg='blue')
points(at.seq[4], dCorrdPar.GLEN['W.scalar'], pch=21, cex=2, bg='gold')
points(at.seq[4], dCorrdPar.RAMSEY['W.scalar'], pch=21, cex=2, bg='red')

points(at.seq[5], dCorrdPar.4B['L.scalar'], pch=21, cex=2, bg='green')
points(at.seq[5], dCorrdPar.2C['L.scalar'], pch=21, cex=2, bg='blue')
points(at.seq[5], dCorrdPar.GLEN['L.scalar'], pch=21, cex=2, bg='gold')
points(at.seq[5], dCorrdPar.RAMSEY['L.scalar'], pch=21, cex=2, bg='red')

points(at.seq[6], dCorrdPar.4B['Rm.scalar'], pch=21, cex=2, bg='green')
points(at.seq[6], dCorrdPar.2C['Rm.scalar'], pch=21, cex=2, bg='blue')
points(at.seq[6], dCorrdPar.GLEN['Rm.scalar'], pch=21, cex=2, bg='gold')
points(at.seq[6], dCorrdPar.RAMSEY['Rm.scalar'], pch=21, cex=2, bg='red')

points(at.seq[7], dCorrdPar.4B['time.scalar'], pch=21, cex=2, bg='green')
points(at.seq[7], dCorrdPar.2C['time.scalar'], pch=21, cex=2, bg='blue')
points(at.seq[7], dCorrdPar.GLEN['time.scalar'], pch=21, cex=2, bg='gold')
points(at.seq[7], dCorrdPar.RAMSEY['time.scalar'], pch=21, cex=2, bg='red')

dev.copy2pdf(file='Change_in_correlation_from_reducing_variation_2.pdf')
