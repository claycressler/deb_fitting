...
## Redo the fitting, with Fh fixed at the value specified by Nisbet et al. 2004.
mod <- 'batch'
## Step 1: Evaluate many randomly chosen parameter sets
## Parameters to estimate:
estimated.pars <- c('K','km','eG','eR','v','Rmbar','Imax','eA','E.0','L.0')
## Bound our ignorance of these parameters with a box
box <- cbind(
             lower=c(K=0.1, km=0.01, eG=0.0001, eR=0.0001, v=1, Rmbar=0.001, Imax=0.0005, eA=0.5, E.0=0.00001, L.0=0.5),
             upper=c(K=0.9, km=1, eG=0.01, eR=0.01, v=25, Rmbar=0.1, Imax=0.05, eA=1, E.0=0.0001, L.0=1.2)
             )
set.seed(12340987)
...
fixed.pars <- c(Fh=0.164,R.0=0,F.0=0.1/0.02,L.sd=0.02)
...
save(results, file=paste0('~/Dropbox/Cressler/DEB_fitting/Comparing_fits_batch_vs_constant/',mod,'_fixed_Fh_results-GLEN-AA-stage-one.rda'))
...
for (i in 1:length(results)) {
    if (is.data.frame(results[[i]])) {
        ...
        results2[[i]] <- vals
        save(results2, file=paste0('~/Dropbox/Cressler/DEB_fitting/Comparing_fits_batch_vs_constant/',mod,'_fixed_Fh_results-GLEN-AA-stage-two.rda'))
    }
}
