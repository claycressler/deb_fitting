for (geno in c('113','322','324','413','711','715','725','815','822')) {
    ...
    save(p, file=paste0(geno,'-A_best_fit_parameters.rda'))
}

## For each genotype, plot all of the individual data and the best-fit
## trajectories. Then I can determine which data/parameter sets to
## exclude from further analysis.
require(deSolve)
dyn.load('nondim_deb_take2.so')
for (geno in c('113','322','324','413','711','715','725','815','822')) {
    load(paste0(geno,'-A_best_fit_parameters.rda'))
