...
#define F0 parms[4]
#define Lerr parms[5]
#define phimax parms[6]
#define hP parms[7]
#define eP parms[8]
#define m parms[9]
#define tau parms[10]
#define Perr parms[11]
...
void derivs (int *neq, double *t, double *y, double *ydot) {
  ...
  // ingestion
  double ing = Imax * F/(Fh+F) * pow(Lobs,g) * exp(-a * Z/pow(Lobs,h));
  // mobilization
  double pc = E * (v/L + km) / (1 + kappa*E/W);

  // fraction of resources captured by the parasite
  double phi = phimax * P/(hP + P);

  // balance the equations
  ydot[0] = -ing;
  ydot[1] = rho*eps*V*ing - pc;
  ydot[2] = (1-phi)*kappa*pc - km*W;
  ydot[3] = eP * phi*kappa*pc - m*P;
  ydot[4] = m*P;
