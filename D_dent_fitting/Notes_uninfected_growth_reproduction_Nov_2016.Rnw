...
\frac{dM}{dt} &= \begin{cases} (1-\kappa) P_C - k_m M \text{ if W < W_{mat}}, \\ 0 \text{ if W >= W_{mat}}, \end{cases} \\
\frac{dR}{dt} &= \begin{cases} 0 \text{ if W < W_{mat}}, \\ \frac{(1-\kappa) P_C - k_m M}{E_R} \text{ if W >= W_{mat}}, \end{cases} \\
P_C &= \frac{E (\nu/L + k_m)}{1 + \kappa E/W}, \\
L &= W^{1/3}, \\
...
y$param <- factor(y$param, levels=c("rho","K","km","ER","Wmat","lik"))
...
A quick and dirty look at the parameter estimates suggests that fixing $F_h$ and estimating the other parameters may not be super-helpful (Fig. \ref{fig:Cat-fits-2}).
There is still a \emph{very} strong negative correlation between, e.g., the estimates of $\kappa$ and $E_R$.
In the simulation-recovery experiment, that correlation was broken down somewhat by using the estimate of $E_R$ to inform the initial conditions of the simulation.
That seems to be less true here: the majority of estimates still indicate that $\kappa$ is very large (> 0.5), and the larger the estimate of $\kappa$ is, the smaller the estimate of $E_R$ is.
There is also a very strong negative correlation with assimilation efficiency and $\kappa$.
So basically, what is happening is the following: if assimilation efficiency is low, then to fit the data the estimate of $\kappa$ needs to be low, and the cost of reproduction is higher, meaning that neonates are born at a larger size and thus have less growth that they need to do in order to sexually mature and start reproducing.

However, Martin et al. 2013 also estimate a pretty high value for $\kappa$ - their estimate is something like 0.68, so an estimate of that magnitude would be pretty reasonable.
Martin et al. also had to use somewhat of a kludgy process to get an estimate of the cost of reproduction based on the DEB rules for how offspring grow prior to birth and the estimate of the scaled maturity at birth.
He also makes use of the parameter $g$, which is dimensionless but is related to dimensional DEB parameters as
\begin{equation}
g = \frac{E_G \nu}{\kappa \rho I_{max}}.
\end{equation}
It can be used to replace some stuff in the mobilization equation.
Note also that $\nu/k_m/g = L_m$ is the maximum possible length.
It might be worth considering reparameterizing the model this way to see if it makes any difference.
Note that Martin et al. was not able to estimate $g$, however, nor were they able to reliably estimate $\nu$.
In the end, they fixed the estimate of $g$ to obtain an estimate of $\nu$.

TO DO: rerun the parameter estimation with a much higher estimate of $v$ to see how and if that affects the estimates of the other parameters.

<<fig.width=5, fig.height=5, fig.cap="Pairwise plot of parameter estimates when the estimate of Fh was between 10,000 and 30,000.", label="Cat-fits-2">>=
ggpairs(subset(y, Fh > 10000 & Fh < 30000), c(1:5,8))
...
x1 <- readRDS(file="RDS_files/Growth_reproduction_trajectory_fitting_Cat_fixed_fh.RDS")
## fh from 2000 to 20000 by 500
x2 <- readRDS(file="RDS_files/Growth_reproduction_trajectory_fitting_Cat_fixed_fh_large.RDS")
