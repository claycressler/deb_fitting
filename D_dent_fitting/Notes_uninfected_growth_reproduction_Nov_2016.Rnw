...
x <- readRDS(file="Trajectory_matching_estimates_uninfected_animals_11-29.RDS")
library(plyr)
library(dplyr)
library(GGally)
...
One thing we can look at is how well the estimated parameters correspond with parameters we can ``eyeball'' on the basis of the data.
In particular, we can use the length-weight regression parameters to convert the predicted mass at maturity to a length at maturity and compare that to the observed lengths when reproduction started in the empirical data.

<<>>=
xi <- 0.0018

y$Lmat <- (y$Wmat/xi)^(1/3)

subset(y, lik < min(lik)+2 & conv==0)$Lmat
