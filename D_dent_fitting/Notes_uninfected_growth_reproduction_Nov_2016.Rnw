...
The assumptions that this model makes are the following:
\begin{enumerate}
\item Ingestion rate depends on observed length, not structural length, as in the standard DEB theory.
This allows us to take advantage of our data on feeding to estimate length-dependent feeding parameters.
\item Mobilization flux $P_C$, however, depends on ``structural'' length, which is related to structural weight $W$ according to $W = L^3$, as specified by standard DEB theory.
\item The relationship between weight ($W$) and observed length $L_{obs}$ is given by the empirical length-weight regression, $W=\xi L_{obs}^q$, where $\xi=1.8\times 10^{-3}$ mgC mm$^{-3}$ and $q = 3$.
Those relationships are established for \emph{D. dentifera} in Hall et al. (2009).
We note that in reality, observed weight does not exactly correspond to structural weight as we are assuming here.
More correctly, observed weight likely depends on the sum $W+E$; this assumption can allow $E$ to take biologically unreasonable excursions.
In particular, early fitting attempts often returned estimates where the initial $L_{obs}$ was 5mm because the initial $E$ was very large.
The value of $E$ decayed rapidly, so that by the first empirical observation of length the $L_{obs}$ would be reasonable.

\item The cost of growth ($E_G$) is assumed to be equal to one; this is not necessarily biologically realistic, but early simulation/recovery experiments showed that it is impossible to estimate $E_G$, the cost of reproduction $E_R$, and the allocation to growth $\kappa$ independently.
The fitting algorithm can simply slide these parameters around relative to one another and achieve identical fits.
\item We assume that the investment in reproduction cannot shrink.
\item We assume that the size at birth is equal to the cost of reproduction, and that the reserve level at birth is the value such that reserve density at birth, $E(0)/W(0)$ is equal to the maximum reserve density, $\rho/v$.
