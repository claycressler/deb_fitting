...
## same code, but doesn't waste computational time calculating g and Imax, since fh is fixed
traj_match2 <- function(estpars, fixpars, parorder, transform, obsdata, events, eval.only=FALSE, method="subplex") {
    estpars <- par_transform(pars=estpars, transform=transform)
    if (any(is.na(estpars)))
        opt <- list(params=estpars,
                    lik=NA)
    else if (eval.only==TRUE) {
        x <- obj2(estpars=estpars,
                 data=obsdata,
                 fixpars=fixpars,
                 parorder=parorder,
                 transform=transform,
...
## Objective function to minimize for the structured model
obj2 <- function(estpars, data, fixpars, parorder, transform, events) {
    y0 <- c(F=unname(fixpars["F0"]),
            E=unname(fixpars["Winit"]/2),
            W=unname(fixpars["Winit"]/2),
            R=0)
