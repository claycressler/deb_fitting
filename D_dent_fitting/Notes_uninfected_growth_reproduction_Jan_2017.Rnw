...
plot(out$time, out$Lpred, type='l', lwd=2, col="grey", ylim=c(0,2), xlab="Age", ylab="Length (mm)")
points(unique(data$age), out[unique(data$age)*10+1,"Lpred"], pch=22, bg=1)
for (i in 1:ncol(bootL)) lines(rep(unique(data$age)[i],2), sort(bootL[,i])[c(25,975)], lwd=2)
points(data$age, data$length, col=2)

plot(out$time, out$R, type='l', lwd=2, col="grey", ylim=c(0,60), xlab="Age", ylab="Cumulative eggs")
points(unique(data$age), out[unique(data$age)*10+1,"R"], pch=22, bg=1)
for (i in 1:ncol(bootR)) if(out[unique(data$age)[i]*10+1,"R"] > 0) lines(rep(unique(data$age)[i],2), sort(bootR[,i])[c(25,975)], lwd=2)
points(data$age, data$eggs, col=2)
