...
## This model
model <- "deb_parasite_W_logistic_delay_m"
...
estpars <- c(km=0.073, aP=1e-8, eP=1e10, kP=5e7, m=1e-6, tau=10, Pobs=500)
...
parorder <- c("rho","K","km","v","F0","Lobs","aP","eP","kP","m","tau","P0","Pobs")
...
box <- cbind(lower=c(km=0.073, aP=1e-12, eP=1e6, kP=1e6, m=1e-6, tau=5, Pobs=10),
             upper=c(km=0.073, aP=1e-6, eP=1e12, kP=1e8, m=1e-1, tau=15, Pobs=1000))
...
print("Optimizing")
## optimize from the top 4000 parameter sets
...
## parameters fixed at the uninfected data values
fixpars <- c(K=1, km=0.073, v=10, F0=1e6/30, Lobs=0.102, P0=100)

## parameter order expected by this model of parasite replication
parorder <- c("rho","K","km","v","F0","Lobs","aP","eP","kP","tau","P0", "Pobs")

## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.15, aP=1e-12, eP=1e6, kP=1e6, m=1e-6, tau=5, Pobs=10),
             upper=c(rho=0.15, aP=1e-6, eP=1e12, kP=1e8, m=1e-1, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
...
## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.15, km=0.073, aP=1e-12, eP=1e6, kP=1e6, m=1e-6, tau=5, Pobs=10),
             upper=c(rho=0.15, km=0.073, aP=1e-6, eP=1e12, kP=1e8, m=1e-1, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
