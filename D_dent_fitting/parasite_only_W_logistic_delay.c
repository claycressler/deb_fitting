...
void derivs (int *neq, double *t, double *y, double *ydot, double *yout, int *ip) {
  ...
  double Pm = y[1]; // mature parasites
  ...
  ydot[0] = eP*aP*W*Pi*(1-(Pi+Pm)/(kP*(Pi+Pm))) - eP*aP*Wtau*ytau[0]*(1-(ytau[0]+ytau[1])/(kP*Wtau));
  ydot[1] = eP*aP*Wtau*ytau[0]*(1-(ytau[0]+ytau[1])/(kP*Wtau));
}
