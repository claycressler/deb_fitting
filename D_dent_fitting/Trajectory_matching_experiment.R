...
if (!file.exists("Trajectory_matching_datasets_11-22.RDS")) {
    ...
    for (d in 1:25) {
        ...
        inds <- 1:12
        ## set up data storage for length and eggs
        data$length <- 0
        data$eggs <- 0
        ## the algorithm cannot fit datasets where no reproduction
        ## happens (there is essentially no information about many
        ## parameters in such a dataset), so we force that to false in
        ## the data
        while (all(data$eggs==0)) {
            pars <- c(Imax=22500,
                      Fh=rnorm(1, mean=10000, sd=2000),
                      g=1.45,
                      rho=rnorm(1, mean=0.4, sd=0.08),
                      K=rnorm(1, mean=0.4, sd=0.08),
                      km=rnorm(1, mean=0.1, sd=0.02),
                      v=rnorm(1, mean=10, sd=2),
