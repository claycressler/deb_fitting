...
## This model
model <- "deb_parasite_E_TypeI_delay"
...
estpars <- c(km=0.073, aP=1e-8, eP=1e10, tau=10, P0=100, Pobs=500)
...
parorder <- c("rho","K","km","v","F0","Lobs","aP","eP","tau","P0","Pobs")
...
box <- cbind(lower=c(km=0.073, aP=1e-12, eP=1e6, tau=5, Pobs=10),
             upper=c(km=0.073, aP=1e-6, eP=1e12, tau=15, Pobs=1000))
...
## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.152, aP=1e-12, eP=1e6, tau=5, Pobs=10),
             upper=c(rho=0.152, aP=1e-6, eP=1e12, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
...
## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.152, km=0.073, aP=1e-12, eP=1e6, tau=5, Pobs=10),
             upper=c(rho=0.152, km=0.073, aP=1e-6, eP=1e12, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
