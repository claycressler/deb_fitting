...
for (d in 1:25) {
    par(mfrow=c(1,6), oma=rep(0.25,4), mar=c(4.5,2,0.25,0.25))
    for (i in 1:6) {
        inds <- which(ests[[d]]$lik < (min(ests[[d]]$lik) + 2))
        parameter <- colnames(ests[[d]][i])
        hist(ests[[d]][inds,i], xlab=parameter, main="", freq=TRUE, breaks=length(inds), ylab="")
        abline(v=datasets[[d]]$params[parameter], col=2)
    }
    dev.copy2pdf(file=paste("Trajectory_matching_7-27_dataset_",d,".pdf"))
}
