...
pars <- c(Imax=22500, Fh=10000, g=1.45, rho=0.1, K=0.3, km=0.15, v=10, F0=1e6/30, E0=0.0005, W0=0.0005, Lobs=0.1, Robs=2)
...
## With this fake data in hand, let's write some functions for fitting it!
fixpars <- pars[c("Imax","g","v","F0")]
estpars <- pars[c("Fh","rho","K","km","E0","W0","Lobs","Robs")]
...
tm_obj(par_transform(estpars, transform),
       ...
       parorder,
       transform)
optimizer(estpars,fixpars,parorder,transform,data,eval.only=TRUE,type="trajectory_matching")

## This works pretty f*ing well! And it's super f*ing fast!
optimizer(estpars,fixpars,parorder,transform,data,type="trajectory_matching",method="Nelder-Mead")
