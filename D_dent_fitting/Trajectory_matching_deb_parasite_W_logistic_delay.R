...
## This model
model <- "deb_parasite_W_logistic_delay"
...
estpars <- c(km=0.073, aP=1e-8, eP=1e10, kP=5e7, tau=10, Pobs=500)
...
box <- cbind(lower=c(km=0.073, aP=1e-12, eP=1e6, kP=1e6, tau=5, Pobs=10),
             upper=c(km=0.073, aP=1e-6, eP=1e12, kP=1e8, tau=15, Pobs=1000))
...
guesses[order(guess_lik)[1:4000]] -> refine
mclapply(refine,
         optimizer,
         fixpars=fixpars,
         parorder=parorder,
...
## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.15, aP=1e-12, eP=1e6, kP=1e6, tau=5, Pobs=10),
             upper=c(rho=0.15, aP=1e-6, eP=1e12, kP=1e8, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
...
## initial guesses at the estimated parameter values
box <- cbind(lower=c(rho=0.15, km=0.073, aP=1e-12, eP=1e6, kP=1e6, tau=5, Pobs=10),
             upper=c(rho=0.15, km=0.073, aP=1e-6, eP=1e12, kP=1e8, tau=15, Pobs=1000))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=500000) %>%
                apply(., 1, as.list) %>%
