...
with(out, plot(time, Lobs, type='l'))
...
fixpars <- c(rho=0.152, K=1, km=0.073, v=10, F0=1e6/30, Lobs=0.0102, eP=1/(0.48*33e-9))
## unconstrained scale transformation
transform <- rep('log',2)
parorder <- c("rho","K","km","v","F0","Lobs","aP","eP","Pobs")

## Estimate the parameters
box <- cbind(lower=c(aP=1e-10, Pobs=1),
             upper=c(aP=1e-4, Pobs=100))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
            nseq=300) %>%
                apply(., 1, as.list) %>%
...
## Revealed parasite manipulation?
## Okay, pretty great. This works well.
box <- cbind(lower=c(aP=1e-10, Pobs=1),
             upper=c(aP=1e-4, Pobs=100))
sobolDesign(lower=box[,'lower'],
            upper=box[,'upper'],
...
guesses[order(guess_lik)[1:150]] -> refine
