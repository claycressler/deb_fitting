...
Using the code above, I generated a set of simulated data using the structured model, and then fit the structured model to that data to look at the best-fit parameter estimates.
...
This also explains why $r$ is so poorly estimated - there is simply not enough information in spore count data to estimate anything about host energy.
...
So, for the moment, I will assume that this parameter non-identifiability is a problem that will go away somewhat with better data.
