...
I do make one simplification from the standard DEB model, in that I do not assume that there are any ``maturity maintenance'' costs; this prevents the possibility of a sexually mature animal ``reverting'' to sexual immaturity.
The model is then:
\begin{align}
