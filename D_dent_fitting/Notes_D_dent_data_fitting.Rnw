...
<<echo=F,fig.width=6, fig.height=4.5, units='in', fig.cap='Feeding model parameters for uninfected animals as $f_h$ is varied. Black points show the maximum likelihood parameter estimates.', label='feeding'>>=
library(tidyr)
library(ggplot2)
library(plyr)
...
<<echo=F,fig.width=6, fig.height=4.5, units='in', fig.cap='Feeding model parameters for infected animals as $f_h$ is varied. Black points show the maximum likelihood parameter estimates for each value of $f_h$.', label='inf-feeding'>>=
library(tidyr)
library(ggplot2)
...
<<echo=F, fig.height=2.5, fig.width=4, units='in', fig.cap="Estimates of $I_{max}$ as $f_h$ varies for uninfected animals (black line), for infected animals when all feeding parameters are estimated simultaneously (blue), and for infected animals when $g$ takes the same value as for uninfected animals (red).", label="Imax-comp">>=
...
Let's compare the observed data against what is predicted by the model for both infected and uninfected animals.
Here, for animals that were exposed but not counted, I will assume their feeding parameters are identical to those of infected animals that were clearly infected.
There are some potential problems with that assumption, but I think it is the correct assumption.
<<echo=FALSE>>=
## the actual data
...
ggplot(uninf_data, aes(x=clearance, y=E.clearance)) +
    geom_point() +
...
p1 <- readRDS("Feeding_model_fitting_infecteds_spore_dependent.RDS")[[50]] %>% subset(., lik==min(lik))
...
library(tidyr)
gather(uninf_data, rate, type, 11:12) -> uninf_data
