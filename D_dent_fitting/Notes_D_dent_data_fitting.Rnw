...
lapply(1:length(fitpars), function(i) subset(fitpars[[i]], lik < min(lik)+2 & conv==0) %>% mutate(., best=ifelse(lik==min(lik),1,0), v=v_vals[i])) -> fitpars
fitp <- vector()
for (i in 1:length(fitpars)) fitp <- rbind(fitp, fitpars[[i]])
library(tidyr)
fitp <- gather(fitp, var, val, 1:4)
...
levels(fitp$var) <- c(paste0("K=",unname(pars["K"])),
                      ...
                      paste0("sigma_L=",unname(pars["Lobs"])))
library(ggplot2)
ggplot(fitp, aes(x=v, y=val, col=best)) +
    facet_wrap(~var, scales="free") +
        geom_point() +
            theme_bw()
@
