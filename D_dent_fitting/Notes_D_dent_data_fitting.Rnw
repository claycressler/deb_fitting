...
<<echo=F, fig.width=6, fig.height=3.5, fig.cap="Parameter estimates for smaller, fixed values of v." label="growth-reprod-3">>=
library(tidyr)
library(ggplot2)
truepars <- datasets[[1]]$params[c("K","km","ER","Lobs")]
...
<<echo=F, fig.height=2, fig.width=2, fig.cap="Very small likelihood peak at low v values.", fig.cap="v-lik-peak">>=
x <- readRDS("Growth_reproduction_trajectory_fitting_profile_lik_v_refine.RDS")
...
plot(c(1:20, seq(20,2000,20)), -liks, type='l', xlim=c(1,200), ylim=c(-118,-117.8))
