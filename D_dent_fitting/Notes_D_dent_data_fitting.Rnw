...
<<echo=F, label="dyn-food-pairwise", fig.width=6, fig.height=3.5, fig.cap="Pairwise scatterplot of estimates of $f_h$ and DEB parameters when $v=10$.">>=
...
<<echo=FALSE, label=dyn-food-fh-est-profile-v>>=
...
<<echo=F, label="estimating-rho", fig.width=4, fig.height=4, units='in', out.width="0.6\\textwidth", fig.cap="Scatterplot of parameter estimates when $\rho$ is estimated along with $f_h$ and the other DEB parameters. $v$ was fixed at $v=100$ for this estimation.">>=
 x <- readRDS("Estimating-rho.RDS")
subset(x, lik < min(lik)+2 & conv==0) %>% mutate(., lik=-lik) %>% ggpairs(., 1:7)
@
...
x <- readRDS("~/Dropbox/Growth_reproduction_trajectory_fitting_dyn_food_profile_rho.RDS")
...
With that in mind, I redid the fitting, assuming that the
