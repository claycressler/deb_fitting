...
<<echo=F, fig.height=4, fig.width=4, out.width='0.5\\textwidth', fig.keep='high', fig.cap='Comparing growth and reproduction trajectories for the true parameters (red) and the best-ftting parameters (black), with the observed data shown by the points.'>>=
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
lapply(1:length(x),
       function(i)
           subset(x[[i]], conv==0) %>%
