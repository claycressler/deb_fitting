...
The results are not particularly encouraging, frankly.
Some of the parameters are fairly tightly estimated at the highest likelihood parameter sets; in particular, $\rho=0.15$, $\kappa=0.077$, and $L_{obs}$ = 0.088.
...
My assumption is that this is only possible because the food is just ``piling up'' so the density never gets low enough to be even remotely limiting to \emph{Daphnia} feeding.
