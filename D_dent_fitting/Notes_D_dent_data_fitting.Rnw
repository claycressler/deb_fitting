...
\end{equation}
Here $\rho$ is the fraction of ingested food which is assimilated, $I_{max}$ is the surface-area-specific ingestion rate, $F$ is the density of food, $f_h$ is the half-saturation constant, and $L^2$ is surface area related to structural length.
...
Based on this, 0.1 mgC/L is $2.5 \times 10^6$ cells/L.
The concentration of algae added to the beaker each day was $3.33 \times 10^7$.
With this initial concentration, $F/(f_h + F) \approx 0.93$; with this value of $f_h$, the concentration of algae would have to stay above $10^7$ cells, suggesting that it might be problematic to assume that
...
One thing to consider: if all the parasite's replication happens early, then the change in feeding rate through infection time has more to do with the \emph{development} of the parasite spores than their numbers (see discussion above).
...
You can see from the profile likelihood for $v$ that a huge range of $v$ values are almost equally supported by the data.
The question is how much you can tighten up the estimates of the other parameters by fixing the value of $v$ at something.
To explore this, I fixed $v$ at a range of values from 1 to 2000 and fit the other four parameters.
The results of this fitting can be seen in the pairwise scatterplots (Fig. \ref{fig:growth-reprod-2}).
...
You can see immediately how much the parameter estimates have tightened up: there is a clear best-fitting parameter set (where there wasn't before).
But you can also see that the estimates of all of the other parameters are highly sensitive to the (fixed) value of $v$.
...
x <- readRDS("~/Dropbox/Growth_reproduction_trajectory_fitting.RDS")
...
mclapply(varv,
    ...
    lapply(., function(x) x$lik) %>%
        unlist -> guess_lik
lik95 <- max(-guess_lik)+0.05*max(-guess_lik)
...
x <- readRDS("~/Dropbox/Growth_reproduction_trajectory_fitting_profile_lik_v.RDS")
v_vals <- c(1,10,50,seq(100,2000,100))
lapply(1:length(x),
       function(i)
           subset(x[[i]], lik==min(lik)) %>%
               mutate(., v=c(1,10,50,seq(100,2000,100))[i], lik=-lik) %>%
                   unlist
       ) %>%
    unlist %>%
        matrix(., ncol=7, byrow=T) -> results
colnames(results) <- c("K","km","ER","Lobs","lik","conv","v")
results[,c("lik","K","km","ER","Lobs","v")] -> results
pairs(results)
@

However, this is all well and good, but the question is whether any of this will hold if the feeding model is wrong.
