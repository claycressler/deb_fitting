...
The other thing that is useful here is to compare the best-fitting trajectories to the datasets and see what characteristics determine whether a fit is good or not.

<<echo=F, fig.height=3.5, fig.width=6, fig.cap='Comparing growth and reproduction trajectories for the true parameters (red) and the best-ftting parameters (black), with the observed data shown by the points.'>>=
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
lapply(1:length(x),
       function(i)
           subset(x[[i]], conv==0) %>%
...
par(mfrow=c(4,2), mar=c(1,4,1,0), oma=c(3,1,0,1))
for (i in 1:4) {
...
ode(y0,times=times,func="derivs",parms=bpars,dllname="deb2",initfunc="initmod",events=list(data=eventdat)) %>%
    ...
    mtext(side=2, line=2.5, 'Eggs')
    legend(x='topleft', paste("Dataset",i), bty='n')
    if(i%%4==0) mtext(side=1, outer=T, line=1, "Age")
}
