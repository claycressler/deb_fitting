...
<<echo=FALSE, label="profile-lik-v-estimating-rho", fig.width=3.5, fig.height=6, fig.cap="Estimates of DEB parameters across a range of $v$ values, when $E_R$ is fixed at its true value.">>=
v_vals <- c(seq(10, 100, 10), seq(200, 1000, 100))
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_profile_v_est_rho.RDS")
lapply(1:length(x),
