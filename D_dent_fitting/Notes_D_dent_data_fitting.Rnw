...
x <- readRDS("~/Downloads/Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets_take_2.RDS")


x2 <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
lapply(1:length(x),
       function(i)
           subset(x[[i]], conv==0) %>%
...
You can see that this didn't make any difference in the fitting (Fig. \ref{fig:tight-fh-mult-datasets}).
The black points are the parameter estimates from the original fitting; the gray points are the estimates from the fitting with a tighter bound on $f_h$.
The red line shows the true parameter value; the dashed black line shows the best-fitting parameter estimate from the weakly constrained fitting, and the green line shows the parameter estimate from the strongly constrained fitting.
This just shows the results for 4 of the 8 datasets, but the results for the other 4 datasets show the exact same pattern.

<<echo=FALSE, fig.height=6.5, fig.width=6, fig.cap='Plotting the parameter estimates when $f_h$ was weakly constrained (black points and line) and when it was strongly constrained (gray points and green line). You can see that these essentially lie atop one another, suggesting that tightening the constraint does not improve the estimates', label='tight-fh-mult-datasets'>>=
d <- c(3, 8, 9, 10, 12, 17, 18, 20)
x1 <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
x2 <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets_2.RDS")

par(mfrow=c(4,4), oma=c(3,3,1,1), mar=c(2,2,1,1))
for (i in c(3, 8, 9, 10)) {
#for (i in c(12,17,18,20)) {
    y1 <- x1[[i]] %>% subset(., conv==0 & lik < min(lik)+5) %>% mutate(., lik=-lik)
    y2 <- x2[[i]] %>% subset(., conv==0 & lik < min(lik)+5) %>% mutate(., lik=-lik)
    for (var in c('fh','rho','K','km')) {
        with(y2, plot(get(var), xlab='', ylab='', lik, pch=21, bg=gray(0.5), col=gray(0.5)))
        with(y1, points(get(var), lik))
        abline(v=datasets[[i]]$params[var], col=2, lwd=2)
        abline(v=y1[1,var], col=3, lwd=2)
        abline(v=y2[1,var], col=1, lwd=2, lty=3)
        if (var=='fh') mtext(side=2, line=2.5, paste("Dataset",i))
        if (i==10) mtext(side=1, line=2.5, var)
...
pars <- c(Imax=22500, # fixed based on feeding data fitting and fh value
          fh=10000, # estimated from fitting growth/reproduction data
          g=1.45, # fixed based on feeding data fitting and fh value
          rho=0.5, # estimated from fitting growth/reproduction data
