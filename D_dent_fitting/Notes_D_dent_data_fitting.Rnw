...
<<echo=FALSE, fig.height=3, fig.width=7, units='in', fig.cap="Comparison of structured (red) and unstructured (teal) model AIC across 40 parameter sets. Lower AIC is better here.", label=aiccomp>>=
est_params <- readRDS("Comparing_structured_unstructured_model_fits.RDS")
...
For a lark, I ran two parameter sets with much lower observation error, to see if this helped tighten up any of the other parameter estimates (Fig. \ref{fig:low-obs-sd}-\ref{fig:low-obs-sd-2}).
