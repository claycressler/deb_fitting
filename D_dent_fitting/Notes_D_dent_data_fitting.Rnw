...
<<echo=F, fig.height=rep(2.5,20), fig.width=rep(7.5,20), fig.keep='high'>>=
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
for (i in 1:20) {
    y <- x[[i]] %>% subset(., conv==0 & lik < min(lik)+2) %>% mutate(., lik=-lik)
    par(mfrow=c(1,4), oma=c(1,3,1,1), mar=c(4,2,1,1))
    for (var in c('fh','rho','K','km')) {
        with(y, plot(get(var), lik, xlab=var, ylab=''))
        abline(v=datasets[[i]]$params[var], col=2)
        abline(v=y[1,var], col=3)
        mtext(side=2, line=1, outer=T, paste("Dataset",i,"lik"))
    }
}
...
<<echo=F, label="mult-datasets-fitting-2", fig.height=3.5, fig.width=6, fig.cap='Parameter estimates across 20 different parameter sets. The black point shows the best-fitting parameter estimate. The grey points are parameter estimates within 2 log-likelihood units of the best. The red point is the true parameter value. For these fits, $v$ was fixed at 10.'>>=
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets_v=10.RDS")
lapply(1:length(x),
       function(i)
           subset(x[[i]], conv==0) %>%
               subset(., lik < min(lik)+2) %>%
...
<<echo=F, label='mult-datasets-relerr', fig.height=3.5, fig.width=6, fig.cap='Relative error in the parameter estimates across the 20 datasets.'>>=
df1 <- subset(d2, type!=2)
df2 <- subset(d2, type==2)[,c("dataset","var","val")]
colnames(df2)[3] <- "truth"
df3 <- subset(d2, type==1)[,c("dataset","var","lik")]
colnames(df3)[3] <- "best"
join(df1, df2, by=c("dataset","var")) %>%
    join(., df3, by=c("dataset","var")) %>%
        mutate(., relerr=abs(val-truth)/truth, likdiff=lik-best) -> df4

ggplot(df4, aes(x=dataset, y=relerr, colour=factor(type))) +
    geom_point() +
        facet_wrap(~var, scales="free") +
            theme_bw() +
                theme(legend.position="none") +
...
<<echo=F, label='truth-est-corr', fig.width=6, fig.height=3.5, fig.cap='Relationship between the true parameter value and the relative error in the best-fitting estimate across 20 datasets.'>>=
ggplot(subset(df4,type==1), aes(x=truth, y=relerr, colour=factor(dataset))) +
    geom_point() +
        facet_wrap(~var, scales="free") +
            theme_bw() +
                theme(legend.position='none')
...
<<echo=F, fig.height=rep(5,20), fig.width=rep(5,20), out.width='0.6\\textwidth', fig.keep='high'>>=
x <- readRDS("Growth_reproduction_trajectory_fitting_dyn_food_multiple_datasets.RDS")
lapply(1:length(x),
       function(i)
           subset(x[[i]], conv==0) %>%
...
for (i in 1:20) {
    ...
    par(mfrow=c(2,2), mar=c(3.5, 3.5, 1, 1), oma=c(0,0,2.5,0))
    plot.new()
    ...
    mtext(side=3, line=0, outer=T, paste("Dataset",i))
    mtext(side=1, line=2, "Age")

    plot.new()
    legend(x='topleft',
           c(paste("var(fh) =", signif(subset(d, dataset==i)$fh %>% var,3)),
             paste("var(rho) =", signif(subset(d, dataset==i)$rho %>% var,3)),
             paste("var(K) =", signif(subset(d, dataset==i)$K %>% var,3)),
             paste("var(km) =", signif(subset(d, dataset==i)$km %>% var,3)),
             paste("var(Lobs) =", signif(subset(d, dataset==i)$Lobs %>% var,3))
             ),
           bty='n'
           )
    ...
    plot.new()
    legend(x='topleft',
           c(paste("fh diff =", signif(unname(subset(d, dataset==i & type==1)$fh-datasets[[i]]$params["fh"]), 3)),
             paste("rho diff =", signif(unname(subset(d, dataset==i & type==1)$rho-datasets[[i]]$params["rho"]), 3)),
             paste("K diff =", signif(unname(subset(d, dataset==i & type==1)$K-datasets[[i]]$params["K"]), 3)),
             paste("km diff =", signif(unname(subset(d, dataset==i & type==1)$km-datasets[[i]]$params["km"]), 3)),
             paste("Lobs diff =", signif(unname(subset(d, dataset==i & type==1)$Lobs-datasets[[i]]$params["Lobs"]), 3))
             ),
           bty='n'
           )
