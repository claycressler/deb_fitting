...
When this happened, the best-fitting parameter set was almost always far from the truth (e.g., for datasets
...
d <- gather(d, var, val, 1:5)
d <- arrange(d, type)

ggplot(d, aes(x=dataset, y=val, colour=factor(type))) +
    geom_point() +
        facet_wrap(~var, scales="free") +
            theme_bw() +
                theme(legend.position="none") +
