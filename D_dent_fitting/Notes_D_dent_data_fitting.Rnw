...
## I am going to work in carbon units.
## rho is the assimilation efficiency (dim.) - based on D. pulex
## Imax is the length-specific maximum clearance rate (ml/(L^g * day)) - based on Spencer fit
## F is the algal concentration (cells/ml) - empirical
## eps is the carbon content per cell (mgC/cell) - based on C content of Chlamy
## g is the scaling of clearance rate with length (dim.) - based on Spencer fit
## Thus, rho*eps*Imax*F*L^g has units of mgC cell^-1 ml mm^-g d^-1 cell ml^-1 mm^g = mgC d^-1
## xi and q are parameters to convert length into dry weight carbon (W=xi*L^q) - based on Nisbet et al. 2004
## K is the fraction of carbon allocated to growth - McCauley et al. 2008
## km is the somatic maintenance rate (1/d) - McCauley et al. 2008
## EG is the carbon required to produce a unit of length (mgC/mm)
## ER is the carbon required to produce a new offspring (mgC) - McCauley et al. 2008
## v is the energy conductance (mm/d)
system("R CMD SHLIB deb.c")
dyn.load("deb.so")
## this produces numbers that look reasonable
...
## length is observed at days c(5,10,12,15,18,25,30,35)
## assume it is observed in 12 animals per day and that these animals have identical parameters
pars <- c(rho=0.7, eps=40e-9, Imax=0.0126, g=1.37, F=1e5/0.01, xi=2.62e-3, q=2.4, K=0.6, km=0.33, ER=1.51e-3, v=10, Lobs=0.1)
ode(y=c(E=0.0002, W=0.0005, R=0),
    times=seq(0, 35),
    func="derivs",
