...
est_params <- readRDS("Comparing_structred_unstructured_model_fits.RDS")
for (i in 1:10) est_params[[i]]$str = est_params[[i]]$str %>% mutate(., t_rep=50*t_rep)
rbind(
    lapply(datasets, function(i) i$params[2:10]) %>%
        unlist %>%
            data.frame(par=names(.), val=., parset=rep(seq(1:10), each=9), kind="truth"),
    lapply(est_params, function(i) i$str[1,1:9]) %>%
        unlist %>%
            data.frame(par=names(.), val=., parset=rep(seq(1:10), each=9), kind="estimate")
    ) -> pars

pars$val[pars$par%in%c("aC","aG","b","hC","hG","m","r")] <- log(pars$val[pars$par%in%c("aC","aG","b","hC","hG","m","r")])
levels(pars$par)=c("log(aC)","log(aG)","log(b)","log(hC)","log(hG)","log(m)","obs_sd","log(r)","t_rep")
