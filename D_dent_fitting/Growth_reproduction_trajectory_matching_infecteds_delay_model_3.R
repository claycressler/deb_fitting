...
delay_model_1 <- function(t, y, parms) {
    ...
    dF <- -ing
    dE <- rho*eps*V*ing - pc
    dW <- K*pc - km*W - aP*W*P1
    if (t < tau) {
        dP1 <- eP*aP*W*P1*(1-P1/(kP*W))
        dP2 <- 0
    }
    else {
        lag <- lagvalue(t-tau)
        dP1 <- eP*aP*W*P1*(1-P1/(kP*W))-eP*aP*lag[3]*lag[4]*(1-lag[4]/(kP*lag[3])) ## maturation
        dP2 <- eP*aP*lag[3]*lag[4]*(1-lag[4]/(kP*lag[3]))
    }
