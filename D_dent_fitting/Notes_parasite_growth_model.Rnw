...
<<'transient-gigantism', fig.width=3, fig.cap="A comparison of best fitting growth trajectories for infected and uninfected animals.">>=
...
try(dede(y0,
         ...
         events=list(data=eventdat))) %>% as.data.frame -> out
xi <- 1.8e-3; q <- 3;
mutate(out, Wobs=W+E, Lobs=(Wobs/xi)^(1/q)) -> inftraj
infboot <- sapply(inftraj[unique(data$age)*10+1,"Lobs"], function(m) rnorm(1000, mean=m, sd=pars["Lobs"]))
...
ode(y,
    ...
    initfunc="initmod",
    events=list(data=eventdat)) %>% as.data.frame -> out
mutate(out, Wpred=W+E, Lpred=(Wpred/1.8e-3)^(1/3)) -> uninftraj
uninfboot <- sapply(uninftraj[unique(data$age)*10+1,"Lpred"], function(m) rnorm(1000, mean=m, sd=pars["Lobs"]))
...
mutate(out0, Lobs=((W+E)/0.0018)^(1/3)) -> out0
...
mutate(out1, Lobs=((W+E)/0.0018)^(1/3)) -> out1
...
try(ode(y0,
        ...
        dllname=model,
        initfunc="initmod",
        events=list(data=eventdat))) -> out2
mutate(as.data.frame(out2), Lobs=((W+E)/0.0018)^(1/3)) -> out2

png(file="~/Desktop/Infected_growth.png", height=3, width=3.5, units='in', res=300)
par(mar=c(3.5, 3.5, 0.5, 0.5), oma=rep(0,4), las=1)
plot(out0$time, out0$Lobs, type='l', lwd=3, ylim=c(0.3,2), xlab="", ylab="", cex.lab=1.2, cex.axis=1, lty=2, col=gray(0.6))
lines(out1$time, out1$Lobs, lwd=3, col=gray(0.3), lty=3)
lines(out2$time, out2$Lobs, lwd=3, col=gray(0))
with(data, points(age, length))
legend(x='bottomright', lwd=2, lty=c(3,2,1), col=rev(c(1,gray(0.3),gray(0.6))), c("Ingestion only", "Ingestion & \ngrowth allocation", "All processes"), bty='n', cex=0.75)
mtext(side=1, line=2.5, "Age", las=0)
mtext(side=2, line=2.5, "Length (mm)", las=0)
...
colnames(data) <- c("Age","Infected","Length", "Clearance", "Offspring", "Spores")
data[,5] = as.character(data[,5])
data[as.character(data[,5])=="n/a",5] = 0
data[,5] <- as.numeric(data[,5])
data[is.na(data[,6]),6] <- 0

## best fitting model of uninfected growth
x10 <- readRDS("Profile_lik_Fh_uninfected_animals_5-11.RDS")
x50 <- readRDS("Profile_lik_Fh_uninfected_animals_v=50_5-11.RDS")
x100 <- readRDS("Profile_lik_Fh_uninfected_animals_v=100_5-11.RDS")
x = list(x10,x50,x100)
...
## best fitting model of infected growth
models <- c("deb_parasite_E_TypeI",
            "deb_parasite_E_TypeI_m",
            "deb_parasite_E_logistic",
            "deb_parasite_E_TypeI_delay",
...
estpars <- estimates[[12]][1,1:(ncol(estimates[[12]])-4)] %>% unlist
parorder <- c(c("rho","K","km","v","F0","Lobs"),names(estpars))
parorder <- parorder[-which(duplicated(parorder))]
pars <- c(fixpars, estpars)
pars <- pars[match(parorder, names(pars))]
...
try(dede(y0,
         ...
         events=list(data=eventdat))) %>% as.data.frame -> out
xi <- 1.8e-3; q <- 3;
mutate(out, Wobs=W+E, Lobs=(Wobs/xi)^(1/q)) -> inf_out

png(file="~/Desktop/All_data_and_predictions.png", width=8, height=2, units='in', res=300)
par(mfrow=c(1,4), mar=c(3.5,4,0.5,0.5), oma=rep(0,4), las=1)
## Plot the observed clearance rate and the prediction from the model
plot(uninf_data$day-0.25, uninf_data$clearance, xlab='', ylab="", cex.axis=1, cex.lab=1.5, cex=1.25)
with(ddply(uninf_data, .(day), summarize, Eclear=mean(E.clearance), sdclear=sd(E.clearance)),
     lines(day, Eclear, lwd=3))
points(inf_data$day+0.25, inf_data$clearance, col=2, cex=1.25)
with(ddply(inf_data, .(day), summarize, Eclear=mean(E.clearance), sdclear=sd(E.clearance)),
     lines(day, Eclear, lwd=3, col=2))
mtext(side=1, line=2.25, "Age", cex=0.8, las=0)
mtext(side=2, line=3, "Foraging rate", cex=0.8, las=0)

with(subset(data, Infected==0), plot(Age-0.25, Length, cex=1.25, xlab="", ylab="", cex.axis=1, cex.lab=1.5))
with(subset(data, Infected==1), points(Age+0.25, Length, cex=1.25, col=2))
with(uninf_out, lines(time[seq(1,351,10)], Lpred[seq(1,351,10)], lwd=3))
with(inf_out, lines(time[seq(1,351,10)], Lobs[seq(1,351,10)], lwd=3, col=2))
mtext(side=1, line=2.25, "Age", cex=0.8, las=0)
mtext(side=2, line=2.5, "Length (mm)", cex=0.8, las=0)

## reproduction data
with(subset(data, Infected==0), plot(Age, Offspring, cex=1.25, xlab="", ylab="", cex.axis=1, cex.lab=1.5))
with(subset(data, Infected==1), points(Age, Offspring, cex=1.25, col=2))
with(uninf_out, lines(time[seq(1,351,10)], R[seq(1,351,10)], lwd=3))
lines(1:35, rep(0, 35), lwd=3, col=2)
mtext(side=1, line=2.25, "Age", cex=0.8, las=0)
mtext(side=2, line=2.5, "Total reproduction", cex=0.8, las=0)

## spore data
with(subset(data, Infected==1), plot(Age, Spores/1e5, cex=1.25, xlab='', ylab="", cex.axis=1, cex.lab=1.5, col=2))
with(inf_out, lines(time[seq(1,351,10)], Pm[seq(1,351,10)]/1e5, lwd=3, col=2))
mtext(side=1, line=2.25, "Age", cex=0.8, las=0)
mtext(side=2, line=2.5, expression(italic('Pasteuria')%*%10^5), cex=0.8, las=0)
