...
\section{22 July 2013}
Returning to this problem again, after so long.

Leah Johnson and I chatted at EEID. Her very strong feeling was that
the choice of feeding model makes a huge difference for the estimates
of every other parameter. I need to see to what extent that is
correct. That is the first order of business.

Bill had the interesting idea of fitting several different models of
ontogeny, ranging in complexity. We then look at the parameter
covariation within each model, and across the models, to understand
how the inferences drawn about life history covariation change. Leah
also mentioned, and I cannot believe that I forgot this, that with
constant feeding rate (as we used for the initial parameters), the
growth model is exactly the von Bertalanffy model (once the reserve
density hits its equilibrium value, anyway). Duh. No wonder it can be
reduced to so few parameters when you non-dimensionalize.

So, what to do?
\begin{enumerate}
  \item Write down dimensional (not non-dimensional) versions of the
    DEB model with both types of feeding (chemostat and batch
    culture).
  \item Use trajectory matching to fit Adriana's data from the high food
    treatment.
  \item Look at the parameter estimates for each individual and see
    how they differ using the two feeding models.
  \item There is also the issue of the starvation rules. Right now, I
    am using a model where $\kappa$ does not vary - if individuals
    cannot meet maintenance, they shrink. If there is time, I really
    need to see how changing these starvation rules might affect the
    model fitting.
\end{enumerate}

I have created a new directory, ``Comparing_fits_batch_vs_chemostat''
and put everything I am working on in that directory.

Some particulars about this fitting:
\begin{enumerate}
\item Bill had come up with the bright idea of keeping track of two lenth
  variables: ``structural'' length and ``measured length''. The
  dynamics of structural length are exactly as described by DEB
  theory. Measured length follows the dynamics of structural length as
  long as growth rate is positive. If growth rate is ever negative,
  structural length will decrease but measured length will stay
  constant. I had forgotten that was how we were dealing with
  starvation. It is an effective approach, for sure.
\item I cannot use the nondimensionlized version of the model because
  the batch culture model would need a new nondimensionalization. This
  means that the parameters would not have the same interpretation, so
  it would be very difficult to understand the patterns of covariation
  among the parameters.
\end{enumerate}

In the new directory, there are two .c files for doing the actual
simulation, deb\_batch.c and and deb\_constant.c.
\end{document}
