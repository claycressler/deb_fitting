...
load('~/Dropbox/Cressler/DEB_fitting/Batch_culture_simulation_recovery/estimation_results_1_stage_three.rda')
res <- tm.vary.results
...
\begin{figure}
<<echo=F,fig=T>>=
plot(tres[,'pam'],tres[,'Fh'],xlab='pam',ylab='Fh')
points(log(true.pars['pam']), log(true.pars['Fh']), col=2)
@
\end{figure}
...
tres <- res
tres[,'nu'] <- log(res[,'nu'])
ttrue.pars <- true.pars
ttrue.pars['nu'] <- log(true.pars['nu'])
par(mfrow=c(3,2))
for (j in i) {
  if (names(res)[j]=='nu') {
    hist(tres[,j]-ttrue.pars[names(res)[j]],
         main=paste(names(res)[j]),
         xlab='')
    abline(v=0, col=2)
    abline(v=mean(tres[,j])-ttrue.pars[names(res)[j]], col=4)
    abline(v=median(tres[,j])-ttrue.pars[names(res)[j]], col=3)
  }
  else {
...
hist(res[,'kappa']/res[,'eG'],
     main='kappa/eG', xlab='', xlim=c(0,800),breaks=1000)
#hist(res[res$loglik<(min(res$loglik)+5),'kappa']/res[res$loglik<(min(res$loglik)+5),'eG'],main='', xlab='', xlim=c(0,800),breaks=1000, add=T, col=2)
...
hist(res[,'km']*res[,'eG'],
     main='km*eG', xlab='')
#hist(res[res$loglik<(min(res$loglik)+5),'km']*res[res$loglik<(min(res$loglik)+5),'eG'],main='', xlab='',col=2,add=T)
...
hist(res[,'Rmbar']/res[,'eR'],
     main='Rmbar/eR', xlab='')
hist(res[res$loglik<(min(res$loglik)+5),'Rmbar']/res[res$loglik<(min(res$loglik)+5),'eR'], col=2,  add=T)
...
<<echo=F,fig=T>>=
hist(res[,'eG']/res[,'eR'],
     main='eG/eR', xlab='',breaks=60)
abline(v=true.pars['eG']/true.pars['eR'], col=2)
@
\end{figure}
...
\begin{figure}
<<echo=F,fig=T>>=
thres <- res[-which(res[,'eG']*res[,'nu']/(res[,'kappa']*res[,'pam']) > 20),]
hist(thres[,'eG']*thres[,'nu']/(thres[,'kappa']*thres[,'pam']),
     main='g', xlab='')
...
\begin{figure}
<<echo=F,fig=T>>=
thres <- res[-which(res[,'kappa']*res[,'pam']/(res[,'km']*res[,'eG']) > 10),]
hist(thres[,'kappa']*thres[,'pam']/(thres[,'km']*thres[,'eG']), main='Lm', xlab='',breaks=15)
